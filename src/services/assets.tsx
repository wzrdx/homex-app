import Midjourney from '../assets/backgrounds/Midjourney_2.jpg';
import Elder from '../assets/backgrounds/Elder.png';
import Elder_2 from '../assets/backgrounds/Elder_2.png';
import Elder_3 from '../assets/backgrounds/Elder_3.png';
import Elder_4 from '../assets/backgrounds/Elder_4.png';
import Frame from '../assets/frame.png';
import Frame_2 from '../assets/frame_2.png';
import Praying from '../assets/images/energy.jpg';

export const getLayoutBackground = () => Elder_4;
export const getFrame = () => Frame_2;
export const getFaucetImage = () => Praying;
